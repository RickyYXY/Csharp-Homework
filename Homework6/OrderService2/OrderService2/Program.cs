...
namespace OrderService2
{
    ...
    public class OrderService
    {
        ...
        {
            orders.Clear();
            return orders.ToArray().Length == 0;
        }
        ...
        public List<Order> FindOrder(string input, string key)
        {
            if(key=="c") //customer search
            {
                var query = orders.Where(o => o.Customer == input)
                    .OrderBy(order => order.SumCost());
                if (query.Count() == 0)
                    return null;
                return query.ToList();
            }
            else //itemname search
            {
                var query = orders.Where(o => o.items.Exists(item => item.Name == input))
                                .OrderBy(order => order.SumCost());
                if (query.Count() == 0)
                    return null;
                return query.ToList();
        ...
        public void ExportOrders(string filename)
        {
            ...
            {
                xmlSerializer.Serialize(fout, orders);
            }
        }
        public void ImportOrders(string filename)
        {
            XmlSerializer xmlSerializer = new XmlSerializer(typeof(List<Order>));
            using (FileStream fin = new FileStream(filename, FileMode.OpenOrCreate))
            {
                orders = (List<Order>)xmlSerializer.Deserialize(fin);
            }
        }
    }
}
